...
# Create output directory name based on CSV filename (removing .csv extension)
output_dir="${cellLine_read_info_file%.csv}_subsampled"
...
for cellline in "${!reads_required[@]}"; do
    ...
    output_file="${output_dir}/${cellline}_subsampled.fastq"
...
cat "${output_dir}"/*_subsampled.fastq > "${output_dir}/combined_subsampled.fastq"

echo "Done! Combined file created at: ${output_dir}/combined_subsampled.fastq"